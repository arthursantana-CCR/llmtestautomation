...
async function getFileTransformFunction(filePath) {
	...
	const fullPath = require_esm.safeJoin(require_logger.cliState_default.basePath || "", actualFilePath);
...
async function getTransformFunction(codeOrFilepath, inputType) {
	...
	if (codeOrFilepath.startsWith("file://")) try {
		...
		require_logger.logger_default.error(`Error loading transform function from file: ${error instanceof Error ? error.message : String(error)}`);
		throw error;
	}
	else try {
		transformFn = getInlineTransformFunction(codeOrFilepath, inputType);
	} catch (error) {
		require_logger.logger_default.error(`Error creating inline transform function: ${error instanceof Error ? error.message : String(error)}`);
		throw error;
	}
	return transformFn;
}
...
//# sourceMappingURL=transform-DshYLyBq.cjs.map