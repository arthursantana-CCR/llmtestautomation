...
async function tryWindowsWhere() {
	try {
		...
		if (!output) {
			require_logger.logger_default.debug("Windows 'where python' returned empty output");
			return null;
		}
		...
		require_logger.logger_default.debug(`Windows 'where python' failed: ${errorMsg}`);
		if (errorMsg.includes("Access is denied") || errorMsg.includes("EACCES")) require_logger.logger_default.warn(`Permission denied when searching for Python: ${errorMsg}`);
	}
	return null;
}
/**
...
async function tryPythonCommands(commands) {
	for (const cmd of commands) try {
		...
		const errorMsg = error instanceof Error ? error.message : String(error);
		require_logger.logger_default.debug(`Python command "${cmd}" failed: ${errorMsg}`);
		if (errorMsg.includes("Access is denied") || errorMsg.includes("EACCES") || errorMsg.includes("EPERM")) require_logger.logger_default.warn(`Permission denied when trying Python command "${cmd}": ${errorMsg}`);
	}
	return null;
}
/**
...
async function runPython(scriptPath, method, args, options = {}) {
	...
	try {
		...
		require_logger.logger_default.debug(`Running Python wrapper with args: ${require_logger.safeJsonStringify(args)}`);
		await new Promise((resolve, reject) => {
			try {
				const pyshell = new python_shell.PythonShell("wrapper.py", pythonOptions);
				pyshell.stdout?.on("data", (chunk) => {
					require_logger.logger_default.debug(chunk.toString("utf-8").trim());
				});
				pyshell.stderr?.on("data", (chunk) => {
					require_logger.logger_default.error(chunk.toString("utf-8").trim());
				});
				pyshell.end((err) => {
					if (err) reject(err);
					else resolve();
		...
		require_logger.logger_default.debug(`Python script ${absPath} returned: ${output}`);
		let result;
		try {
			result = JSON.parse(output);
			require_logger.logger_default.debug(`Python script ${absPath} parsed output type: ${typeof result}, structure: ${result ? JSON.stringify(Object.keys(result)) : "undefined"}`);
		} catch (error) {
...
//# sourceMappingURL=pythonUtils-rOCm9w_5.cjs.map