...
import { A as BeavertailsPlugin, C as HarmbenchPlugin, D as DebugAccessPlugin, E as DivergentRepetitionPlugin, F as retryWithDeduplication, I as sampleArray, L as fetchHuggingFaceDataset, M as RedteamGraderBase, N as RedteamPluginBase, O as CrossSessionLeakPlugin, P as getCustomPolicies, S as ImitationPlugin, T as ExcessiveAgencyPlugin, _ as makeInlinePolicyIdSync, a as UnverifiableClaimsPlugin, b as OverreliancePlugin, c as ToolDiscoveryPlugin, ct as readProviderPromptMap, d as RbacPlugin, f as PromptExtractionPlugin, g as isValidPolicyObject, h as determinePolicyTypeFromId, i as VLGuardPlugin, it as getDefaultProviders, j as AegisPlugin, k as ContractPlugin, l as SqlInjectionPlugin, m as PolicyPlugin, o as UnsafeBenchPlugin, ot as processPrompts, p as PoliticsPlugin, r as VLSUPlugin, rt as doRemoteGrading, s as ToxicChatPlugin, st as readPrompts, t as GRADERS, u as ShellInjectionPlugin, v as PlinyPlugin, w as HallucinationPlugin, x as IntentPlugin, y as getPiiLeakTestsForCategory } from "./graders-CwtelA9k.js";
import { i as generateIdFromPrompt } from "./utils-GiyI2K4P.js";
import { a as sha256, i as randomSequence, n as isBlobStorageEnabled } from "./extractor-BA90EtLq.js";
import "./genaiTracer-foKtbVa2.js";
import "./chat-CVOyOBYk.js";
...
import "./messages-BMkwrfmt.js";
import "./util-BZaMVBMq.js";
import "./openai-Cv9pEKxp.js";
import "./util-DHBpsbZE.js";
import "./completion-BLe7SUA4.js";
import { n as getBlobUrl, t as getBlobByHash } from "./blobs-MlVQyoSc.js";
import { a as evalResultsTable, c as evalsToPromptsTable, d as promptsTable, g as getDb, h as closeDbIfOpen, i as datasetsTable, l as evalsToTagsTable, n as blobReferencesTable, o as evalsTable, p as tagsTable, r as configsTable, s as evalsToDatasetsTable, t as blobAssetsTable, u as modelAuditsTable } from "./tables-BjaApSAB.js";
import { n as escapeRegExp, t as ellipsize } from "./text-DuYSUYPB.js";
import { n as getTraceStore } from "./store-BFLqwuc_.js";
import "./base-DHam07ip.js";
import "./image-C1i3NoZo.js";
import "./providerRegistry-D32Lt9vp.js";
import "./rubyUtils-Dn6MGcsk.js";
import { c as readAssertions, l as renderMetricName, o as doesPromptRefMatch, s as assertions_default, t as evaluate$1, u as runAssertions } from "./evaluator-gmF3Vddp.js";
import { n as setupSignalWatcher, t as readSignalEvalId } from "./signal-DM_SPNmi.js";
import { a as createEvalId, n as EvalQueries, s as getEvalSummaries, t as Eval } from "./eval-WaSg64w3.js";
...
function runCommand(program) {
	program.command("run").description("Scan code changes for LLM security vulnerabilities").argument("[repo-path]", "Repository path to scan", ".").option("--api-key <key>", "Promptfoo API key for authentication").option("--base <ref>", "Base branch or commit to compare against").option("--compare <ref>", "Compare branch or commit").option("-c, --config <path>", "Path to config file").option("--api-host <url>", "Promptfoo API host URL (default: https://api.promptfoo.app)").option("--diffs-only", "Scan only PR diffs, skip filesystem exploration").option("--json", "Output results as JSON").option("--github-pr <owner/repo#number>", "GitHub PR to post comments to").option("--min-severity <level>", "Minimum severity level (low|medium|high|critical)").option("--minimum-severity <level>", "Alias for min-severity (low|medium|high|critical)").option("--guidance <text>", "Custom guidance for the security scan").option("--guidance-file <path>", "Path to file containing custom guidance").action(async (repoPath, cmdObj) => {
		...
		const { executeScan } = await import("./scanner-C28XVEq2.js");
	...
	exportCmd.command("logs").description("Collect and zip log files for debugging").option("-n, --count <number>", "Number of recent log files to include (default: all)").option("-o, --output [outputPath]", "Output path for the compressed log file").action(async (cmdObj) => {
		try {
			...
			if (!fs$1.existsSync(logDir)) {
				logger_default.error(`No log directory found. Logs are created when running commands like "promptfoo eval".\nLog directory: ${logDir}`);
				process.exitCode = 1;
				return;
			}
			const allLogFiles = getLogFilesSync();
			if (allLogFiles.length === 0) {
				logger_default.error(`No log files found in the logs directory. Logs are created when running commands like "promptfoo eval".\nLog directory: ${logDir}`);
				process.exitCode = 1;
				return;
			}
...
function registerRedteamRunTool(server) {
	server.tool("redteam_run", {
		...
		try {
			const { configPath, output, force = false, maxConcurrency = DEFAULT_MAX_CONCURRENCY, delay, filterPrompts, filterProviders, remote = false, progressBar = true } = args;
			try {
	...
	server.tool("run_evaluation", {
		...
		try {
			...
			if (testCaseIndices !== void 0 || promptFilter || providerFilter) {
				...
				const { evaluate } = await import("./evaluator-DwS5NAEj.js");
				const evalRecord = await (await import("./eval-0IFf8hON.js")).default.create(config, filteredTestSuite.prompts, { id: `mcp-eval-${Date.now()}` });
...
async function main() {
	...
	program.version(VERSION).showHelpAfterError().showSuggestionAfterError().on("option:*", function() {
		...
		if (unknownArgs.length > 0) logger_default.error(`Invalid option(s): ${unknownArgs.join(", ")}`);
		else logger_default.error("Invalid option(s)");
		program.help();
		process.exitCode = 1;
	});
