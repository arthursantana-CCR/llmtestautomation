...
function getWrapperDir(type) {
	...
	require_logger.logger_default.debug(`Resolved ${type} wrapper directory: ${result}`);
	return result;
}
/**
...
async function importModule(modulePath, functionName) {
	require_logger.logger_default.debug(`Attempting to import module: ${JSON.stringify({
		resolvedPath: safeResolve(modulePath),
		moduleId: modulePath
	})}`);
	try {
		if (modulePath.endsWith(".ts") || modulePath.endsWith(".mjs")) {
			require_logger.logger_default.debug("TypeScript/ESM module detected, importing tsx/cjs");
			await import("tsx/cjs");
		}
		const resolvedPath = (0, node_url.pathToFileURL)(safeResolve(modulePath));
		const resolvedPathStr = resolvedPath.toString();
		require_logger.logger_default.debug(`Attempting ESM import from: ${resolvedPathStr}`);
		...
		require_logger.logger_default.debug(`Successfully imported module: ${JSON.stringify({
			resolvedPath,
			moduleId: modulePath
		})}`);
		if (functionName) {
			require_logger.logger_default.debug(`Returning named export: ${functionName}`);
		...
		if (modulePath.endsWith(".js") && isCjsInEsmError(errorMessage)) {
			require_logger.logger_default.debug(`ESM import failed for ${modulePath}, attempting vm-based CJS fallback: ${errorMessage}`);
			try {
				const resolvedPath = safeResolve(modulePath);
				const mod = loadCjsModule(resolvedPath);
				require_logger.logger_default.debug(`Successfully loaded module via CJS fallback: ${JSON.stringify({
					resolvedPath,
					moduleId: modulePath
				})}`);
				if (functionName) {
				...
				require_logger.logger_default.error(`ESM import failed for ${modulePath}: ${errorMessage}`);
				require_logger.logger_default.error(`CJS fallback also failed: ${cjsErrorMessage}`);
		...
		const e = err;
		if (e.stack) require_logger.logger_default.debug(e.stack);
		if (err.code === "ERR_MODULE_NOT_FOUND") {
			const resolvedModulePath = safeResolve(modulePath);
			try {
				await node_fs_promises.default.access(resolvedModulePath);
				require_logger.logger_default.error(`ESM import failed: ${err}`);
			} catch {
		...
		} else require_logger.logger_default.error(`ESM import failed: ${err}`);
		throw err;
	}
}
/**
